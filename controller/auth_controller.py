...
def login():
    if request.method == "POST":
        ...
        if not correo or not password:
            return render_template("login.html", error="Correo y contraseña son obligatorios")

        # Validación: formato básico de email
        if '@' not in correo:
            return render_template("login.html", error="Ingresa un correo electrónico válido")
